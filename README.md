...
URL is included directly in every Lua sequence. On the first MoisturePilot run,
the client automatically verifies the device using FarmBot's built-in
authorization token. MoisturePilot does not store that token; it creates a
separate device credential and stores it on FarmBot automatically. The user
does not find an ID, receive a key, create an account, or retain a code.
...
The sequence automatically enrolls the device when necessary, follows FarmBot's calibrated
photo grid, captures each image with its position, reports progress, and asks
the service to finalize the result. The Lua client derives a custom grid with
50% image overlap on both axes from FarmBot's calibrated camera footprint. The
service uses FarmBot coordinates as absolute placement anchors, aligns reliable
neighboring overlaps, and produces a garden mosaic,
